...
class Dreamer(nn.Module):
    def __init__(self, obs_space, act_space, config, logger, dataset):
        ...
        self._wm = models_slots.WorldModel(obs_space, act_space, self._step, config)
        self._task_behavior = models_slots.ImagBehavior(
    ...
    def _policy(self, obs, state, training):
        if state is None:
            ...
            latent = self._wm.dynamics.initial(len(obs["is_first"]))
        ...
        embed = self._wm.encoder(obs)["slots"]
...
def make_env(config, mode, slot_shape=None):
    ...
    if slot_shape is not None:
        env = wrappers.SlotObservation(env, slot_shape)
    return env
