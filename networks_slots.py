...
class RSSM(nn.Module):
    def __init__(
        ...
        self._cell = GRUGNNCell(self._deter, self._get_stochastic_dim(), self._num_actions, self._gnn_hidden_dim, self._slot_dim[0])
        ...
        self._img_out_layers_objectwise = nn.Sequential(*img_out_layers)
        self._img_out_layers_objectwise.apply(tools.weight_init)

        obs_out_layers = []
        if self._temp_post:
            inp_dim = self._deter + self._slot_dim[1]
        else:
            inp_dim = self._slot_dim[1]
        for i in range(self._layers_output):
            ...
            if i == 0:
                inp_dim = self._hidden
        self._obs_out_layers_objectwise = nn.Sequential(*obs_out_layers)
        self._obs_out_layers_objectwise.apply(tools.weight_init)

        if self._discrete:
            self._ims_stat_layer_objectwise = nn.Linear(self._hidden, self._stoch * self._discrete)
            self._ims_stat_layer_objectwise.apply(tools.weight_init)
            self._obs_stat_layer_objectwise = nn.Linear(self._hidden, self._stoch * self._discrete)
            self._obs_stat_layer_objectwise.apply(tools.weight_init)
        else:
            self._ims_stat_layer_objectwise = nn.Linear(self._hidden, 2 * self._stoch)
            self._ims_stat_layer_objectwise.apply(tools.weight_init)
            self._obs_stat_layer_objectwise = nn.Linear(self._hidden, 2 * self._stoch)
            self._obs_stat_layer_objectwise.apply(tools.weight_init)
    ...
    def _get_stochastic_dim(self):
        stochastic_dim = self._stoch
        if self._discrete:
            stochastic_dim *= self._discrete

        return stochastic_dim

    def initial(self, batch_size):
        num_objects = self._slot_dim[0]
        deter = torch.zeros(batch_size, num_objects, self._deter).to(self._device)
        if self._discrete:
            state = dict(
                logit=torch.zeros([batch_size, num_objects, self._stoch, self._discrete]).to(
                    self._device
                ),
                stoch=torch.zeros([batch_size, num_objects, self._stoch, self._discrete]).to(
                    self._device
                ),
                deter=deter,
            )
            ...
            state = dict(
                mean=torch.zeros([batch_size, num_objects, self._stoch]).to(self._device),
                std=torch.zeros([batch_size, num_objects, self._stoch]).to(self._device),
                stoch=torch.zeros([batch_size, num_objects, self._stoch]).to(self._device),
                deter=deter,
            )
    ...
    def get_feat(self, state):
        ...
        if self._discrete:
            dims = stoch.size()[:-2]
            stoch = stoch.reshape(*dims, self._get_stochastic_dim())
    ...
    def obs_step(self, prev_state, prev_action, embed, is_first, sample=True):
        ...
        if self._shared:
            ...
            batch_size, num_objects = prior["deter"].size()[:2]
            if self._temp_post:
    ...
    def img_step(self, prev_state, prev_action, embed=None, sample=True):
        ...
        # (batch, num_objects, stoch, discrete_num)
        x = prev_state["stoch"]
        batch_size, num_objects = x.size()[:2]
        # (batch, num_objects, stoch, discrete_num) -> (batch, num_objects, stoch * discrete_num)
        x = x.reshape(batch_size, num_objects, self._get_stochastic_dim())

        if self._shared:
            if embed is None:
                embed = torch.zeros(batch_size, *self._slot_dim)
            # (batch, num_objects, stoch * discrete_num) -> (batch, num_objects, stoch * discrete_num + embed)
            x = torch.cat([x, embed], dim=-1)

        for _ in range(self._rec_depth):  # rec depth is not correctly implemented
            # (batch, num_objects, stoch * discrete_num), (batch, num_actions), (batch, num_objects, deter) -> (batch, num_objects, deter), (batch, num_objects, deter)
            x, deter = self._cell(x, prev_action, prev_state["deter"])
    ...
    def get_stoch_objectwise(self, deter):
        x = self._img_out_layers_objectwise(deter)
        stats = self._suff_stats_layer_objectwise("ims", x)
        dist = self.get_dist(stats)
        return dist.mode()

    def _suff_stats_layer_objectwise(self, name, x):
        if self._discrete:
            if name == "ims":
                x = self._ims_stat_layer_objectwise(x)
            elif name == "obs":
                x = self._obs_stat_layer_objectwise(x)
            else:
...
class MultiEncoder(nn.Module):
    def __init__(
        ...
        self.slot_shapes = {
            k: v
            for k, v in shapes.items()
            if re.match(slot_keys, k)
        }
        assert len(self.slot_shapes) <= 1, 'Only one slot-based observation is allowed!'
        print("Encoder CNN shapes:", self.cnn_shapes)
        print("Encoder MLP shapes:", self.mlp_shapes)
        print("Slot shapes:", self.slot_shapes)
        ...
        self.slot_dim = None
        if self.slot_shapes:
            self.slot_dim = list(self.slot_shapes.values())[0]
    ...
    def forward(self, obs):
        ...
        return {'flat_embedding': outputs, 'slots': obs['slots'].detach()}
...
class ActionHeadGNN(nn.Module):
    def __init__(self,
        ...
        self._gnn = GNN(input_dim, hidden_dim, action_dim=None, num_objects=num_objects, ignore_action=True, copy_action=False, use_interactions=True, edge_actions=False, output_dim=hidden_dim)
        self._dist = mlp_dist
        act = getattr(torch.nn, mlp_act)
        norm = getattr(torch.nn, mlp_norm)
        self._min_std = mlp_min_std
        self._max_std = mlp_max_std
        self._init_std = mlp_init_std
        self._unimix_ratio = mlp_unimix_ratio
        self._temp = mlp_temp() if callable(mlp_temp) else mlp_temp

        mlp = [norm(hidden_dim, eps=1e-03).apply(tools.weight_init), act()]
        if self._dist in ["tanh_normal", "tanh_normal_5", "normal", "trunc_normal"]:
            mlp.append(nn.Linear(hidden_dim, 2 * output_dim).apply(tools.uniform_weight_init(mlp_outscale)))
        elif self._dist in ["normal_1", "onehot", "onehot_gumbel"]:
            mlp.append(nn.Linear(hidden_dim, output_dim).apply(tools.uniform_weight_init(mlp_outscale)))

        self._dist_layer = nn.Sequential(*mlp)

    def forward(self, features, dtype=None):
        batch_dims = features.size()[:-2]
        gnn_dims = features.size()[-2:]
        x = self._gnn(features.reshape(math.prod(batch_dims), *gnn_dims), action=None)[0]
        x = torch.mean(x, dim=1).reshape(*batch_dims, -1)
        if self._dist == "tanh_normal":
            ...
            x = self._dist_layer(x)
            mean, std = torch.split(x, [self._output_dim] * 2, -1)
...
class GRUGNNCell(nn.Module):
    def __init__(self, deterministic_dim, stochastic_dim, action_dim, gnn_hidden_dim, num_objects, act=torch.tanh, update_bias=-1):
        super(GRUGNNCell, self).__init__()
        self._deterministic_dim = deterministic_dim
        self._stochastic_dim = stochastic_dim
        self._action_dim = action_dim
        self._gnn_hidden_dim = gnn_hidden_dim
        self._num_objects = num_objects
        self._act = act
        self._update_bias = update_bias
        self._gnn = GNN(deterministic_dim + stochastic_dim, gnn_hidden_dim, action_dim, num_objects,
                        ignore_action=False, copy_action=True, layer_norm=True, num_layers=3, use_interactions=True,
                        edge_actions=True, output_dim=3 * deterministic_dim)

    def forward(self, stochastic, action, state):
        full_state = torch.cat([state, stochastic], dim=-1)
        parts = self._gnn(full_state, action)[0]
...
class GNN(torch.nn.Module):
    ...
    def forward(self, states, action, edge_weight=None):
...
class GNNAggregation(nn.Module):
    def __init__(self, input_dim, hidden_dim, action_dim, num_objects, ignore_action=False, copy_action=False,
                 act_fn="relu", layer_norm=True, num_layers=3, use_interactions=True, edge_actions=False,
                 output_dim=None, mlp_act="SiLU", mlp_norm="LayerNorm", mlp_dist="normal", mlp_outscale=1.0,
                 mlp_device="cuda"):
        super().__init__()
        if output_dim is None:
            output_dim = input_dim

        self._gnn = GNN(input_dim, hidden_dim, action_dim, num_objects, ignore_action, copy_action, act_fn, layer_norm,
                        num_layers, use_interactions, edge_actions, hidden_dim)
        self._mlp = MLP(inp_dim=hidden_dim, shape=output_dim, layers=1, units=hidden_dim, act=mlp_act, norm=mlp_norm,
                        dist=mlp_dist, outscale=mlp_outscale, device=mlp_device)

    def forward(self, states, action=None, edge_weight=None):
        batch_dims = states.size()[:-2]
        gnn_dims = states.size()[-2:]
        node_attr = self._gnn(states.reshape(math.prod(batch_dims), *gnn_dims), action, edge_weight)[0]
        node_attr = torch.mean(node_attr, dim=1)

        return self._mlp(node_attr.reshape(*batch_dims, -1))
